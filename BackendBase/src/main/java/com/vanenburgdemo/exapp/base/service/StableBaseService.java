...
public class StableBaseService<T extends StableBase> extends BaseBusinessLogic<T>
		implements IStableBaseService<T> {
		
	private static final Logger LOGGER = LoggerFactory.getLogger(StableBaseService.class.getName());
	...
	@Autowired
	private  StablePerimeterBaseImpl stablePerimeterBaseImpl;
	
	
	public StableBaseService(Class<T> modelClass) {
		super(modelClass);
	...
	public void isObjectExists(T modelObj, boolean isUpdate) {
		T existing = getByField("sid", modelObj.getSid());
		
		if(isUpdate) {			
			if(existing == null || existing.getSid().equals(modelObj.getSid())) {
				return;
			}
			throw new AlreadyExistsException(ErrorCode.ALREADY_EXISTS, "Stable", modelObj.getSid());
	...
	public void onAfterSaveDB(T modelObj) {
	  changelogService.createChangeLog("Stable", modelObj.getSid(), Constants.SAVED, modelObj);
	}
	...
	public void onAfterUpdateDB(T modelObj) {
	  changelogService.createChangeLog("Stable", modelObj.getSid(), Constants.UPDATED, modelObj);
	}
	...
	public void onAfterDeleteDB(T modelObj) {
	  changelogService.createChangeLog("Stable", modelObj.getSid(), Constants.DELETED, modelObj);
	}	
	...
	protected IPerimeterManager<T> getPerimeterManager() {
		return stablePerimeterBaseImpl;
	}
}