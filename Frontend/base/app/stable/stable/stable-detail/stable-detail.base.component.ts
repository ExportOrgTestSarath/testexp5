...
import { StableApiConstants } from '@baseapp/stable/stable/stable.api-constants';
...
export class StableDetailBaseComponent{
...
confirmationReference:any;
  ...
  "actions" : {
	...
	pageViewTitle: string = 'STABLE_DETAIL';
	
	public stableService = inject(StableService);
  ...
  if (formControl.controls[parent] instanceof FormGroup){
	...
	editFormula(config:any,fieldname:any){
     ...
     if(fieldConfig.formulaMethod){
       let methodName: Exclude<keyof StableDetailBaseComponent, ' '> = fieldConfig.formulaMethod
    ...
    let action: Exclude<keyof StableDetailBaseComponent, ' '> = methodName;
...
step:workflowInfo.step
  ...
  const requestedObj = this.generateRequestObj(data);
      ...
      const saveSubscription = this.uploadAttachmentsandSaveData(requestedObj, splittedData,actionName).subscribe(res => {
        ...
        const saveSubscription = this.stableService[method](requestedObj,actionName).subscribe((res:StableBase) => {
          this.onAfterSave(res, data, method, isToastNotNeeded);
        })
        this.subscriptions.push(saveSubscription);
      }
...
uploadAttachmentsandSaveData(data: any, splittedData: any,actionName?:string): Observable<any> {
    ...
    if (!this.id) {
       const saveSubscription =  this.stableService.create(splittedData.data,actionName).subscribe(
        createdData => {
    ...
    const subject$ = new Subject();    
      const updateSubscription = this.deleteFiles(splittedData).subscribe((responseData: any) => {
        this.saveFiles(responseData).subscribe((dataToUpdate: any) => {
          ...
          this.stableService.update(dataToUpdate.data,actionName).subscribe(
            res => {
              subject$.next(res);
              subject$.complete();
            },
...
workflowInformation.step = workflowDataForNewRec.step || '';
  ...
  if (environment.prototype && this.id) {
    ...
    this.stableService.getProtoTypingDataById(params).subscribe((res: any) => {
    ...
    const dataSubscription = this.stableService.getById (params).subscribe((res: StableBase[]) => {
      this.data = res || {};
      this.formatRawData();
    });
