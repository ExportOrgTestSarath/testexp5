...
export class StableListBaseComponent{
...
localStorageStateKey = "stable-list";
...
@ViewChild('toggleButton')
 ...
 filtersApplied:boolean = false;
  ...
  "rowHeight" : "medium"
	...
	pageViewTitle: string = 'STABLE_LIST';
  ...
  this.clearFilterValues();
	...
	if (this.tableConfig.detailPage?.url) {
      const value: any = "parentId";
       let property: Exclude<keyof StableListBaseComponent, ''> = value;
       const methodName: any = "onUpdateChild";
       let action: Exclude<keyof StableListBaseComponent, ''> = methodName;
...
focus(){
  ...
  this.tableSearchFieldConfig= this.appUtilBaseService.getControlsFromFormConfig(this.tableSearchConfig)
    ...
    return {
      ...
      ajaxUrl: StableApiConstants.getDatatableData,
      select: true,
 ...
 updateActions() {
  ...
  this.filtersApplied = false;
    ...
    if (this.selectedValues.length > 0) {
      ...
      this.confirmationService.confirm({
        ...
        accept: () => {
          const deleteSubscription = this.stableService.delete(requestedParams).subscribe((res: any) => {
  ...
  this.showAdvancedSearch = !this.showAdvancedSearch;
	...
	if (this.isChildPage && this[property]) {
		...
		this.router.navigate(['../stabledetail'], { relativeTo: this.activatedRoute});
	}
}
	loadGridData() {
    let gridSubscription: any;
    if (environment.prototype && this.tableConfig.children?.length > 0) {
      gridSubscription = this.stableService.getProtoTypingData().subscribe((data: any) => {
